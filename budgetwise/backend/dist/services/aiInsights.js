...
export async function generateDashboardInsights(spendingData, month, year) {
    ...
    const messages = [
        {
            role: "system",
            content: "You are a personal finance advisor for a college student. Return only JSON output using the required structure and no extra prose.",
        },
        {
            role: "user",
    ...
    for (const modelName of uniqueModelCandidates) {
        const candidateModes = STRICT_JSON_SCHEMA_MODELS.has(modelName)
        ...
        for (const outputMode of candidateModes) {
            try {
                ...
                const completion = await client.chat.completions.create({
                    model: modelName,
                    messages: messages,
                    response_format: buildResponseFormat(outputMode),
                    temperature: 0.2,
                });
